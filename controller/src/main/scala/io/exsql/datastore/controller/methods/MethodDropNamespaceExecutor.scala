...
case class MethodDropNamespaceExecutor(private val namespaceHelper: NamespaceHelper) {

  def apply(request: http.Request): Future[http.Response] = request.method match {
    case Method.Options => Protocol.allowResponse(request, Protocol.MethodDropNamespace.requestSchema())
    case _ =>
      Protocol.fromRequest(request, Protocol.MethodDropNamespace.requestSchema()).flatMap { byteGraph =>
        dropNamespace(byteGraph.body.name.as[String]()).map { success =>
          Protocol.httpResponse(
            body = Protocol.MethodDropNamespace.response(
  ...
  private def dropNamespace(name: String): Future[Boolean] = namespaceHelper.drop(name)
