...
case class MethodDeployStreamExecutor(private val namespaceHelper: NamespaceHelper) {

  def apply(request: http.Request): Future[http.Response] = request.method match {
    case Method.Options => Protocol.allowResponse(request, Protocol.MethodDeployStream.requestSchema())
    case _ =>
      Protocol.fromRequest(request, Protocol.MethodDeployStream.requestSchema()).flatMap { byteGraph =>
        deployStream(byteGraph.body).map { success =>
          Protocol.httpResponse(
            body = Protocol.MethodDeployStream.response(
  ...
  private def deployStream(stream: ByteGraph): Future[Boolean] = {
    namespaceHelper.deployStream(stream)
  }
